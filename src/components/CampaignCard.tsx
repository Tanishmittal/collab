...
import type { CampaignEligibilityResult } from "@/lib/campaignEligibility";
...
const CampaignCard = ({
  ...
  const isIneligible = eligibility ? !eligibility.eligible : false;

  const nicheStyle = nicheColors[campaign.niche] || "text-teal-600 border-teal-200 bg-teal-50";
  const requirementChips = [
    ...(campaign.targetPlatforms || []).slice(0, 1).map((platform) => `${platform} required`),
    ...(campaign.minFollowers ? [`${campaign.minFollowers.toLocaleString()}+ followers`] : []),
    ...(campaign.minEngagementRate ? [`${campaign.minEngagementRate}%+ engagement`] : []),
    ...(campaign.portfolioRequired ? ["Portfolio required"] : []),
    ...(campaign.verifiedSocialsOnly ? ["Verified only"] : []),
  ].slice(0, 2);
  const openBrandProfile = async (event: React.MouseEvent) => {
    event.stopPropagation();
    await navigateToBrandProfile(navigate, campaign.userId);
  };
  ...
  return (
    <motion.div
      ...
      onClick={() => navigate(`/campaign/${campaign.id}`, { state: { backTo: "/?tab=campaigns" } })}
    >
      <div className="p-6 flex-grow flex flex-col relative z-10">
          ...
          <div className="mb-5 flex items-start">
            ...
            <button type="button" onClick={openBrandProfile} className="shrink-0">
              <BrandAvatar
                ...
                className="h-14 w-14 shrink-0 rounded-2xl shadow-sm group-hover:scale-105 transition-transform duration-500"
            ...
            {eligibility && (
              <Badge
                className={cn(
                  "ml-auto rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide shadow-none",
                  eligibility.eligible
                    ? "border-emerald-200 bg-emerald-50 text-emerald-700"
                    : "border-amber-200 bg-amber-50 text-amber-700"
                )}
              >
                {eligibility.eligible ? "Eligible" : "Check fit"}
              </Badge>
            )}
          </div>
        </div>
        ...
        {eligibility && (
          <div
            className={cn(
              "mb-5 rounded-2xl border px-3.5 py-3 text-xs font-medium",
              eligibility.eligible
                ? "border-emerald-200 bg-emerald-50 text-emerald-700"
                : "border-amber-200 bg-amber-50 text-amber-700"
            )}
        ...
        <div className="mt-auto">
          ...
          {isOwn ? (
            ...
            <button
              className="w-full py-3 bg-slate-200 text-slate-600 rounded-xl font-bold tracking-wide shadow flex items-center justify-center gap-2 cursor-default"
              disabled
            >
            ...
            <button
              className={cn(
                "w-full py-3 rounded-xl font-bold tracking-wide transition-colors duration-300 shadow flex items-center justify-center gap-2",
                isIneligible
              ...
              onClick={(e) => {
                ...
                navigate(`/campaign/${campaign.id}`, { state: { backTo: "/?tab=campaigns" } });
              }}
              disabled={isIneligible}
            >
              {isIneligible ? "Not Eligible" : "Apply Now"}
