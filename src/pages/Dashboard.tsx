...
const Dashboard = () => {
  ...
  const fetchData = async () => {
    ...
    if (myAppsRes.data) {
      setMyApplications(myAppsRes.data as MyApplicationRow[]);
    }
  ...
  const deleteCampaign = async (campaignId: string) => {
    ...
    if (!error) {
      ...
      setApplications(prev => prev.filter(a => a.campaign_id !== campaignId));
      ...
      <div className="container py-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Manage your campaigns, applications, and bookings</p>
          </div>
          <CreateCampaignModal
            trigger={
        ...
        <Tabs defaultValue="campaigns" className="w-full">
          <TabsList className="w-full grid grid-cols-4 mb-6">
            <TabsTrigger value="campaigns" className="font-display">My Campaigns</TabsTrigger>
            <TabsTrigger value="received-apps" className="font-display">
              Received Apps ({pendingApps.length})
            </TabsTrigger>
            <TabsTrigger value="sent-apps" className="font-display">
              Sent Apps ({myApplications.length})
            </TabsTrigger>
            <TabsTrigger value="bookings" className="font-display">
            ...
            {campaigns.length === 0 ? (
              <Card className="glass-card">
                <CardContent className="p-12 text-center">
                  ...
                  <p className="text-muted-foreground mt-1 text-sm">Create your first campaign to start connecting with influencers.</p>
                  <CreateCampaignModal
                    trigger={
                    ...
                    <motion.div key={c.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
                      <Card className="glass-card hover:shadow-lg transition-shadow">
                        <CardContent className="p-5">
                          <div className="flex items-start justify-between gap-3">
                            <div className="flex items-center gap-3 min-w-0">
                              ...
                              <div className="min-w-0">
                                <Link to={`/campaign/${c.id}`} className="font-display font-semibold text-foreground hover:text-primary transition-colors truncate block">
                                  {c.brand}
                                </Link>
                                ...
                                <DropdownMenuTrigger asChild>
                                  <Badge
                                    ...
                                    className={`cursor-pointer transition-colors ${
                                      c.status === "active" ? "gradient-primary border-0 text-primary-foreground" :
                                      c.status === "paused" ? "border-warning text-warning" : ""
                                    }`}
                                  >
                                    {c.status === "active" && <Play size={10} className="mr-1" />}
                                    {c.status === "paused" && <Pause size={10} className="mr-1" />}
                                    {c.status === "closed" && <Archive size={10} className="mr-1" />}
                                    {c.status}
                                  </Badge>
                                </DropdownMenuTrigger>
                                <DropdownMenuContent align="end" className="w-40">
                                  {c.status !== "active" && (
                                    <DropdownMenuItem onClick={() => toggleCampaignStatus(c.id, "active")} className="gap-2 text-success">
                                      <Play size={14} /> Set Active
                                    </DropdownMenuItem>
                                  )}
                                  {c.status !== "paused" && (
                                    <DropdownMenuItem onClick={() => toggleCampaignStatus(c.id, "paused")} className="gap-2 text-warning">
                                      <Pause size={14} /> Pause
                                    </DropdownMenuItem>
                                  )}
                                  {c.status !== "closed" && (
                                    <DropdownMenuItem onClick={() => toggleCampaignStatus(c.id, "closed")} className="gap-2 text-muted-foreground">
                                ...
                                <AlertDialogContent>
                                  <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>
                                      This will permanently delete "{c.brand}" and all its applications.
                                    </AlertDialogDescription>
                                  </AlertDialogHeader>
                                  <AlertDialogFooter>
                          ...
                          <p className="text-sm text-muted-foreground mt-3 line-clamp-1">{c.description}</p>
                          ...
                          <div className="grid grid-cols-4 gap-3 mt-4">
                            <div className="p-2 rounded-lg bg-muted/50 text-center">
                              <div className="font-semibold text-sm text-foreground flex items-center justify-center gap-0.5">
                                <IndianRupee size={12} />{c.budget.toLocaleString()}
                              </div>
                              <span className="text-xs text-muted-foreground">Budget</span>
                            </div>
                            <div className="p-2 rounded-lg bg-muted/50 text-center">
                              <div className="font-semibold text-sm text-foreground">{campaignApps.length}</div>
                              <span className="text-xs text-muted-foreground">Applied</span>
                            </div>
                            <div className="p-2 rounded-lg bg-muted/50 text-center">
                              <div className="font-semibold text-sm text-foreground">{accepted}</div>
                              <span className="text-xs text-muted-foreground">Accepted</span>
                            </div>
                            <div className="p-2 rounded-lg bg-muted/50 text-center">
                              <div className="font-semibold text-sm text-foreground">{pending}</div>
                              ...
                              <AccordionItem value="applications" className="border-none">
                                <AccordionTrigger className="text-sm py-2 hover:no-underline">
                                  <span className="flex items-center gap-2">
                                    <Users size={16} /> View Received Applications ({campaignApps.length})
                                  </span>
                                </AccordionTrigger>
                                <AccordionContent className="pt-2 pb-0">
                                  <div className="space-y-3">
                                    {campaignApps.map((app) => {
                                      ...
                                      return (
                                        <div key={app.id} className="p-3 rounded-lg border bg-card text-card-foreground shadow-sm">
                                          <div className="flex items-start justify-between">
                                            <div className="flex items-center gap-3">
                                              <div className="w-8 h-8 rounded-full gradient-primary flex items-center justify-center text-primary-foreground font-display font-bold text-xs">
                                                {initials}
                                              </div>
                                              <div>
                                                <InfluencerProfileModal profile={profile}>
                                          ...
                                          {app.status === "pending" && (
                                            <div className="flex gap-2 mt-2">
                                              <Button size="sm" className="h-7 text-xs px-2" onClick={() => updateApplicationStatus(app.id, "accepted")}>Accept</Button>
                                              <Button size="sm" variant="outline" className="h-7 text-xs px-2" onClick={() => updateApplicationStatus(app.id, "rejected")}>Reject</Button>
                                            </div>
                                          )}
                                        </div>
                                      );
            ...
            {applications.length === 0 ? (
              <Card className="glass-card">
                <CardContent className="p-12 text-center">
                  ...
                  <h3 className="font-display font-semibold text-lg text-foreground">No applications yet</h3>
                  <p className="text-muted-foreground text-sm mt-1">Applications from influencers for your campaigns will appear here.</p>
                </CardContent>
              </Card>
            ) : (
              <div className="space-y-3">
                          ...
                          <div className="flex items-center justify-between mt-3">
                            ...
                            {app.status === "pending" && (
                              <div className="flex gap-2">
                                <Button size="sm" className="gradient-primary border-0 text-primary-foreground h-8 px-3"
          ...
          <TabsContent value="sent-apps">
            {myApplications.length === 0 ? (
              <Card className="glass-card">
                <CardContent className="p-10 text-center">
                  <Send size={36} className="mx-auto text-muted-foreground mb-3" />
                  <h3 className="font-display font-semibold text-lg text-foreground">No applications yet</h3>
                  <p className="text-muted-foreground mt-1 text-sm">Browse campaigns and apply to start collaborating with brands.</p>
                  <Button className="mt-4 gradient-primary border-0 text-primary-foreground" onClick={() => navigate("/campaigns")}>
                    Browse Campaigns
                  </Button>
                </CardContent>
              </Card>
                        ...
                        <CardContent className="p-5">
                          <div className="flex items-start justify-between">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <h3 className="font-display font-semibold text-foreground">{campaign?.brand || "Unknown Campaign"}</h3>
                                <div className="flex items-center gap-2 text-xs text-muted-foreground mt-0.5">
                                  {campaign?.city && <span className="flex items-center gap-0.5"><MapPin size={11} /> {campaign.city}</span>}
                                  {campaign?.niche && <><span>·</span><Badge variant="secondary" className="text-[10px] py-0 px-1.5 h-4">{campaign.niche}</Badge></>}
                                  {campaign?.budget && <><span>·</span><span className="flex items-center gap-0.5"><IndianRupee size={10} />{campaign.budget.toLocaleString()}</span></>}
                                </div>
                              </div>
                            </div>
                          ...
                          <Accordion type="single" collapsible className="mt-2 w-full">
                            <AccordionItem value="details" className="border-none">
                              <AccordionTrigger className="text-sm py-2 text-muted-foreground hover:no-underline">
                                View Application Details
                              </AccordionTrigger>
                              <AccordionContent>
                                {app.message && (
                                  <p className="text-sm text-muted-foreground mt-1 bg-muted/30 rounded-lg p-2.5">
                                    "{app.message}"
                                  </p>
                                )}
            ...
            {bookings.length === 0 ? (
              <Card className="glass-card">
                <CardContent className="p-12 text-center">
                  ...
                  <h3 className="font-display font-semibold text-lg text-foreground">No bookings yet</h3>
                  <p className="text-muted-foreground text-sm mt-1">
                    Direct bookings from brands or your booking requests will appear here.
                  </p>
                </CardContent>
              </Card>
            ) : (
              <div className="space-y-3">
                {bookings.map((booking, i) => {
                  const isBrand = booking.brand_user_id === user!.id;
                  const otherName = isBrand ? booking.influencer_name : booking.brand_name;
                    ...
                    <motion.div key={booking.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }}>
                      <Card className="glass-card">
                        <CardContent className="p-4">
                          <div className="flex items-start justify-between">
                            <div className="flex items-center gap-3">
                              <div className="w-10 h-10 rounded-full bg-muted flex items-center justify-center text-foreground font-display font-bold text-sm">
                                {otherName?.charAt(0) || "?"}
                              </div>
                              <div>
                                <div className="font-semibold text-sm text-foreground">
                                  {isBrand ? `Booking for ${booking.influencer_name}` : `Booking from ${booking.brand_name}`}
                                </div>
                                <div className="text-xs text-muted-foreground mt-0.5">
                                  {(booking.items || []).map((item: any) => `${item.qty}x ${item.type}`).join(", ")}
                                </div>
                              </div>
                            </div>
                            <div className="flex items-center gap-2">
                              <span className="font-display font-bold text-sm text-foreground flex items-center">
                                <IndianRupee size={12} />{booking.total_amount.toLocaleString()}
                              </span>
                              <Badge variant={
                          ...
                          <div className="flex items-center justify-between mt-3">
                            ...
                            {booking.status === "pending" && !isBrand && (
                              <div className="flex gap-2">
                                <Button size="sm" className="gradient-primary border-0 text-primary-foreground h-8 px-3"
